...
//2023feb26, spi, begin
int global_inputmidichannel = -1; //-1 for omni mode, listening all midi channels, otherwise between 0 and 15 for listening specific midi channel only
//2023feb26, spi, end
...
void CALLBACK StartGlobalProcess(UINT uTimerID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2)
{
	...
	if(it!=global_inputmididevicemap.end())
	{
		...
		sprintf(pCHAR, "INPUT:\n");StatusAddTextA(pCHAR);
		sprintf(pCHAR, "%s\n", global_inputmididevicename.c_str());StatusAddTextA(pCHAR);
		sprintf(pCHAR, "%d\n\n", global_inputmidichannel);StatusAddTextA(pCHAR);
	...
	if(it!=global_outputmididevicemap.end())
	{
		...
		sprintf(pCHAR, "OUTPUT:\n");StatusAddTextA(pCHAR);
		sprintf(pCHAR, "%s\n", global_outputmididevicename.c_str());StatusAddTextA(pCHAR);
		sprintf(pCHAR, "%d\n\n", global_outputmidichannel);StatusAddTextA(pCHAR);
                     ...
                     int       nCmdShow)
{
	...
	if(nArgs>23)
	{
		global_inputmidichannel = atoi(szArgList[23]);
	}
	//2023feb26, spi, end
	LocalFree(szArgList);
	...
	if(global_inputmidichannel<-1 || global_inputmidichannel>15)
	{
		swprintf(pWCHAR, L"invalid inputmidichannel, input midi channel must range from 0 to 15 inclusively or -1 for omni mode.\n");StatusAddText(pWCHAR);
		return FALSE;
	}
	if(global_outputmidichannel<0 || global_outputmidichannel>15)
	{
		swprintf(pWCHAR, L"invalid outputmidichannel, output midi channel must range from 0 to 15 inclusively.\n");StatusAddText(pWCHAR);
		return FALSE;
	}
